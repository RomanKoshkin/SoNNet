...
int32_t generateRandom32bitInt() {
    // Random device
    std::random_device rd;

    // Mersenne Twister 19937 generator, seeded with rd()
    std::mt19937 gen(rd());

    // Uniform distribution range [INT32_MIN, INT32_MAX]
    ...
    if (fabs(x) <= 1.28) {
        ...
        q = exp(-y) * b0 /
            (fabs(x) - b1 +
             b2 / (fabs(x) + b3 +
                   b4 / (fabs(x) - b5 +
                         b6 / (fabs(x) + b7 - b8 / (fabs(x) + b9 + b10 / (fabs(x) + b11))))));
        //
        //  12.7 < |X|
        //
    } else {
...
//
{
    double a[8] = {3.3871328727963666080,    1.3314166789178437745E+2, 1.9715909503065514427E+3,
    ...
    double c[8] = {1.42343711074968357734,    4.63033784615654529590,   5.76949722146069140550,
    ...
    double e[8] = {6.65790464350110377720,    5.46378491116411436990,    1.78482653991729133580,
...
//
{
    ...
    if (order == 0) {
        ...
        value = pow(mu, 5) + 10.0 * pow(mu, 3) * pow(sigma, 2) + 15.0 * mu * pow(sigma, 4);
    } else if (order == 6) {
        value = pow(mu, 6) + 15.0 * pow(mu, 4) * pow(sigma, 2) + 45.0 * pow(mu, 2) * pow(sigma, 4) +
                15.0 * pow(sigma, 6);
    } else if (order == 7) {
        ...
        value = pow(mu, 8) + 28.0 * pow(mu, 6) * pow(sigma, 2) +
                210.0 * pow(mu, 4) * pow(sigma, 4) + 420.0 * pow(mu, 2) * pow(sigma, 6) +
                105.0 * pow(sigma, 8);
    } else {
        cerr << "\n";
...
#define N_MAX 16
    ...
    static double f_vec[N_MAX] = {1.0,     1.0,      2.0,      3.0,      8.0,    15.0,
                                  48.0,    105.0,    384.0,    945.0,    3840.0, 10395.0,
                                  46080.0, 135135.0, 645120.0, 2027025.0};
...
void truncated_normal_ab_cdf_values(int &n_data, double &mu, double &sigma, double &a, double &b,
                                    double &x, double &fx)
...
#define N_MAX 11
    ...
    static double a_vec[N_MAX] = {50.0, 50.0, 50.0, 50.0, 50.0, 50.0, 50.0, 50.0, 50.0, 50.0, 50.0};
...
//
{
    ...
    for (r = 0; r <= order; r++) {
        ...
        moment = moment + r8_choose(order, r) * pow(mu, order - r) * pow(sigma, r) * ir;
...
void truncated_normal_ab_pdf_values(int &n_data, double &mu, double &sigma, double &a, double &b,
                                    double &x, double &fx)
...
void truncated_normal_a_cdf_values(int &n_data, double &mu, double &sigma, double &a, double &x,
                                   double &fx)
...
void truncated_normal_a_pdf_values(int &n_data, double &mu, double &sigma, double &a, double &x,
                                   double &fx)
...
//
{
    ...
    variance =
        sigma * sigma *
        (1.0 + alpha * alpha_pdf / (1.0 - alpha_cdf) - pow(alpha_pdf / (1.0 - alpha_cdf), 2));
...
void truncated_normal_b_cdf_values(int &n_data, double &mu, double &sigma, double &b, double &x,
                                   double &fx)
...
void truncated_normal_b_pdf_values(int &n_data, double &mu, double &sigma, double &b, double &x,
                                   double &fx)
...
//
{
    ...
    variance = sigma * sigma * (1.0 - beta * beta_pdf / beta_cdf - pow(beta_pdf / beta_cdf, 2));
