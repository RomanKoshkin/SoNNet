...
class cClassOne(object):
    ...
    def __init__(self, NE=-1, NI=-1, NEo=0, cell_id=-1, home=None):
        ...
        self.lib.getF.restype = ndpointer(dtype=np.float64, ndim=1, shape=(N,))
        ...
        self.lib.getD.restype = ndpointer(dtype=c_double, ndim=1, shape=(N,))

        self.lib.setF.argtypes = [c_void_p, ndpointer(dtype=c_double, shape=(N,))]
        self.lib.setF.restype = c_void_p  # returns a void pointer (i.e. because it doesn't have a return statement)

        self.lib.setD.argtypes = [c_void_p, ndpointer(dtype=c_double, shape=(N,))]
        ...
        self.lib.getUU.restype = ndpointer(dtype=np.float64, ndim=1, shape=(N,))

        self.lib.setUU.argtypes = [c_void_p, ndpointer(dtype=c_double, shape=(N,))]
        ...
        self.lib.getTheta.restype = ndpointer(dtype=np.float64, ndim=1, shape=(N,))
    ...
    def calcMeanF(self, _iIDs):
        leniIDs = len(_iIDs)
        iIDs = np.ascontiguousarray(np.zeros((self.N,)).astype('int32'))
